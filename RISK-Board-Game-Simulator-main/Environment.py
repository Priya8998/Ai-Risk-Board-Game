...
class Environment:
    ...
    def display_logs(self,last=1):
        ...
        for i in range(1,last+1):
            ...
            # printing data in tabular form with active player highlighted
            headings=['Player name','Player id','Territories owned','Continents owned','Troops owned']
            data=[ [players_name[p],players_id[p],territories_owned[p],continents_owned[p],troops_owned[p]] for p in range(len(self.players))]

            row_format ="{:>20}"*(len(headings))

            # attempt to use colorama for colored output; fallback to markers if unavailable
            try:
            ...
            for p_idx, row in enumerate(data):
                row_vals = [str(x) for x in row]
                if p_idx == self.curr_player.id:
                    if _use_color:
                        prefix = Fore.GREEN + Style.BRIGHT
                        suffix = Style.RESET_ALL
                        print(prefix + row_format.format(*row_vals) + suffix)
                    else:
                        # simple marker fallback for terminals without color support
