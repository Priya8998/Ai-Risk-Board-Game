...
class Start_with_gui:
    def __init__(self,env, ai_agents=None):
        self.env=env
        self.started=False
    ...
    def update_info(self):
        ...
        # If current player is an AI, let it take its automated turn
        try:
            pid = self.env.curr_player.id
            if pid in self.ai_agents and self.started and not self.completed:
                # run AI turn in small delay to allow UI update
                self.root.after(50, lambda: self.ai_agents[pid].take_turn())
        except Exception:
            pass
...
# Use the script directory as base_dir so bundled map and images are found
env=Environment(base_dir=os.path.dirname(__file__),players=[pl0,pl1])

# instantiate a simple AI for player 1
ai1 = SimpleAI(env, player_id=1)
ai_agents = {1: ai1}

game=Start_with_gui(env, ai_agents=ai_agents)
game.start_render()